...
void generate_level(level_t* level){
    ...
    for(size_t i = 0; i < level->height; i++){
        for(size_t j = 0; j < level->width; j++){
            level->level_arr[i][j] = object_chars[BORDER];
        }
    }
    ...
    for (int i = 0; i < level->height; i++) {
      for (int j = 0; j < level->width; j++) {
        if (level->level_arr[i][j] == object_chars[NOT_GENERATED]) {
          ...
          int limit_i = (di[direction] == 0)
                            ? i
                            : (di[direction] == 1 ? i + length : i - length);
...
void generate_points(level_t* level){
    for(int count = 0; count < level->points;){
        ...
        if(level->level_arr[i][j] == object_chars[PATH]){
            level->level_arr[i][j] = object_chars[FOOD];
            count++;
        }
    }
}
